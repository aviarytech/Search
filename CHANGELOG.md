...
- Jev can use the page for you: ⌘J, say what you want done, and TypeSafe's Jev clicks, types, picks from dropdowns and scrolls its way there, a step at a time, saying what it is doing as it goes; ■ or esc stops it. It types a key at a time, moves the pointer before it clicks and pauses between actions, as a person does; if a site asks whether you're human, it stops and leaves that to you. It only ever types words from what you asked, and never touches a password field. It needs a TypeSafe API key as `TYPESAFE_API_KEY` in `~/.env`, and sends the page's links, buttons and text to TypeSafe while it works. Settings › General › Let Jev use the page, off by default.
- Claude can plan what Jev does: with Settings › General › Let Claude plan it on, ⌘J asks Claude, which reads the page, works out the steps, writes whatever needs typing and hands Jev the clicking, then answers in the bar; ask a follow-up and it carries on from there. When a site is a dead end, or there's no page yet, it starts over from a web search with your search engine, as a person would. It won't buy, send, delete or type passwords unless you asked for exactly that. It needs `ANTHROPIC_API_KEY` in `~/.env` (and `ANTHROPIC_BASE_URL` to go through a server of your own), and sends the page to Anthropic while it works. Off by default.
